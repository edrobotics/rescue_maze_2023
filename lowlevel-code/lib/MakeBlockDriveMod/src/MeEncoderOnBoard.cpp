...
// Move the motor some distance (degrees)
// distance - distance to move in degrees.
// speed - speed to move at in rpm
void MeEncoderOnBoard::speedMove(long distance, double speed) {
  encode_structure.targetPos = encode_structure.currentPos + distance;
  if(distance>=0) runSpeed(speed);
  else runSpeed(-speed);
}
