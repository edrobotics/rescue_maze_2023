...
// Returns which wall to use based on the wall presences
WallSide getWallToUse()
{
...
double leftToTurn(bool zeroCross, int turningDirection, double tarAng, double curAng)
{
  ...
  return -100; // If something went wrong. Normally you should never get -100 (overshoot of 100 degrees). I currently do not check for this in my other code
...
bool driveStepDriveLoop(WallSide& wallToUse, double& dumbDistanceDriven, StoppingReason& stopReason, bool& rampDriven)
{
  ...
  if (abs(g_robotAngle) <= 40) // Only check wallchanges if below certain angle. Correction for angle is also done with the distance.
  {
