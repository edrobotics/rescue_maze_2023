...
Command serialcomm::readCommand(bool waitForSerial)
{
  if (waitForSerial == false)
    return readCommand(waitForSerial, 1000);
  else
...
{
  if (waitForSerial == false && Serial.available() < 1)
    return command_none; // Return none if Serial was not available and you should not wait
  else if (waitForSerial == true)
  {
    long timeFlag = millis();
    while (Serial.available() == 0 && (millis() - timeFlag < timeout))
    {
    } // Wait while no serial is available. When continuing serial will be available
    if (Serial.available() == 0)
      return command_none; // if the timout has passed and serial is still not available
  }
...
RGBColour lights::safeMltp(RGBColour base, double multiplier)
{
  ...
  if (returnColour.red > 255)
    returnColour.red = 255;
  returnColour.green = base.green * multiplier;
  if (returnColour.green > 255)
    returnColour.green = 255;
  returnColour.blue = base.blue * multiplier;
  if (returnColour.blue > 255)
    returnColour.blue = 255;
  return returnColour;
}
...
{
  if (digitalRead(encoderLF.getPortB()) == 0)
    encoderLF.pulsePosMinus();
  else
    encoderLF.pulsePosPlus();
}
void isr_encoderLB(void)
{
  if (digitalRead(encoderLB.getPortB()) == 0)
    encoderLB.pulsePosMinus();
  else
    encoderLB.pulsePosPlus();
}
void isr_encoderRF(void)
{
  if (digitalRead(encoderRF.getPortB()) == 0)
    encoderRF.pulsePosMinus();
  else
    encoderRF.pulsePosPlus();
}
void isr_encoderRB(void)
{
  if (digitalRead(encoderRB.getPortB()) == 0)
    encoderRB.pulsePosMinus();
  else
    encoderRB.pulsePosPlus();
}
...
void runWheelSide(WheelSide wheelSide, double speed)
{
  ...
  {
  } // An error: no valid WheelSide specified
}
...
void centerAngle180(double &refAngle, double &calcAngle)
{
  ...
  calcAngle += angleDiff;
  while (calcAngle >= 360)
    calcAngle -= 360; // Bind calcAngle to valid mathangle interval.
  while (calcAngle < 0)
    calcAngle += 360; // Bind calcAngle to valid mathangle interval.
}
...
{
  if (normalWallPresence[wall_left] && normalWallPresence[wall_right])
    return wall_both;
  else if (normalWallPresence[wall_left])
    return wall_left;
  else if (normalWallPresence[wall_right])
    return wall_right;
  else
    return wall_none; // If no wall was detected
}

WallSide RobotPose::getSafeWallToUse()
{
  if (safeWallPresence[wall_left] && safeWallPresence[wall_right])
    return wall_both;
  else if (safeWallPresence[wall_left])
    return wall_left;
  else if (safeWallPresence[wall_right])
    return wall_right;
  else
    return wall_none; // If no wall was detected
}
...
void RobotPose::calculate2(WallSide wallSide, double &tAngle, bool useGyroAngle, double distanceIncrement)
{
  ...
  if (ultrasonicCurrentDistances[ultrasonic_F][usmt_smooth] < MAX_ULTRASONIC_NEAR)
  {
    ...
    {
      newY += 30;
    } // Wrap the value to 0-30, as yDist should be between 0 and 30 normally
...
void RobotPose::calculate1(WallSide wallSide, double &tAngle, double &distance, bool useGyroAngle)
{
  ...
  if (useGyroAngle == false)
    tAngle = atan((d2 - d1) / ULTRASONIC_SPACING);
  else
    tAngle *= DEG_TO_RAD; // Convert the angle to radians to execute the calculation

  distance = cos(tAngle) * ((d1 + d2) / 2.0);
  if (wallSide == wall_right)
    distance = 30 - distance; // Invert if the measured side is the right one
...
void RobotPose::print()
{
  ...
  Serial.print("  ");
  Serial.print("targetGyroAngle:  ");
  Serial.print(targetGyroAngle, 2);
  Serial.print("  ");
  Serial.print("xDist:  ");
  Serial.print(xDist, 1);
  Serial.print("  ");
  Serial.print("yDist:  ");
  Serial.print(yDist, 1);
  Serial.print("  ");
  Serial.print("distOnRamp:  ");
  Serial.print(distOnRamp, 1);
  Serial.print("  ");
  Serial.println("");
}
...
void gyroTurnSteps(TurningDirection direction, int steps, bool doCorrection, double turningSpeed, bool updatePose)
{
  ...
  if (steps == 0)
    pose.angle = pose.angle + angleDiff;
  else
    pose.angle = pose.angle + angleDiff - multiplier * 90;
...
{
  pose.update();
  gyroTurnSteps(direction, steps, true, turningSpeed, false);
  // Angle is roughly updated by gyroTurnSteps using the gyro
...
void straighten()
{
  gyroTurnSteps(cw, 0, true, BASE_TURNING_SPEED, false);
}
...
void sideWiggleCorrection()
{
  ...
  if (distanceError < 0)
    direction = wall_right; // If to the left, go right
  else
    direction = wall_left; // If to the right, go left
...
void updateDistanceArray(UltrasonicSensorEnum sensorToUse)
{
  ...
  while (ultrasonicRawDistancesPointer[sensorToUse] >= DISTANCE_MEASUREMENT_SIZE)
    ultrasonicRawDistancesPointer[sensorToUse] -= DISTANCE_MEASUREMENT_SIZE;
  while (ultrasonicRawDistancesPointer[sensorToUse] < 0)
    ultrasonicRawDistancesPointer[sensorToUse] += DISTANCE_MEASUREMENT_SIZE;
}
...
void prinSensortWallPresence()
{
  Serial.print("RAW:    ");
  Serial.print("F:");
  Serial.print(currentSensorWallStates[ultrasonic_F][usmt_raw]);
  Serial.print(" LF:");
  Serial.print(currentSensorWallStates[ultrasonic_LF][usmt_raw]);
  Serial.print(" LB:");
  Serial.print(currentSensorWallStates[ultrasonic_LB][usmt_raw]);
  Serial.print(" RF:");
  Serial.print(currentSensorWallStates[ultrasonic_RF][usmt_raw]);
  Serial.print(" RB:");
  Serial.print(currentSensorWallStates[ultrasonic_RB][usmt_raw]);
  Serial.println("");

  Serial.print("SMOOTH: ");
  Serial.print("F:");
  Serial.print(currentSensorWallStates[ultrasonic_F][usmt_smooth]);
  Serial.print(" LF:");
...
void printWallPresence()
{
  Serial.print("NORMAL: ");
  Serial.print("F: ");
  Serial.print(normalWallPresence[wall_front]);
  Serial.print(" L: ");
  Serial.print(normalWallPresence[wall_left]);
  Serial.print(" R: ");
  Serial.print(normalWallPresence[wall_right]);
  Serial.println("");

  Serial.print("SAFE:   ");
  Serial.print("F: ");
  Serial.print(safeWallPresence[wall_front]);
  Serial.print(" L: ");
  Serial.print(safeWallPresence[wall_left]);
  Serial.print(" R: ");
  Serial.print(safeWallPresence[wall_right]);
  Serial.println("");
}
...
{
  // Raw
  if (ultrasonicCurrentDistances[sensor][usmt_raw] < WALL_PRESENCE_TRESHOLD)
    currentSensorWallStates[sensor][usmt_raw] = true;
  else
    currentSensorWallStates[sensor][usmt_raw] = false;

  // Smooth
  if (ultrasonicCurrentDistances[sensor][usmt_smooth] < WALL_PRESENCE_TRESHOLD)
    currentSensorWallStates[sensor][usmt_smooth] = true;
  else
    currentSensorWallStates[sensor][usmt_smooth] = false;
}
...
{
  Serial.print("LF: ");
  Serial.print(ultrasonicTheoreticalDistances[ultrasonic_LF]);
  Serial.print("  ");
  Serial.print("LB: ");
  Serial.print(ultrasonicTheoreticalDistances[ultrasonic_LB]);
  Serial.print("  ");
  Serial.print("RF: ");
  Serial.print(ultrasonicTheoreticalDistances[ultrasonic_RF]);
  Serial.print("  ");
  Serial.print("RB: ");
  Serial.print(ultrasonicTheoreticalDistances[ultrasonic_RB]);
  Serial.print("  ");
  Serial.println("");
}
...
{
  Serial.print("LF: ");
  Serial.print(ultrasonicTheoreticalDistances[ultrasonic_LF] - ultrasonicCurrentDistances[ultrasonic_LF][usmt_smooth]);
  Serial.print("  ");
  Serial.print("LB: ");
  Serial.print(ultrasonicTheoreticalDistances[ultrasonic_LB] - ultrasonicCurrentDistances[ultrasonic_LB][usmt_smooth]);
  Serial.print("  ");
  Serial.print("RF: ");
  Serial.print(ultrasonicTheoreticalDistances[ultrasonic_RF] - ultrasonicCurrentDistances[ultrasonic_RF][usmt_smooth]);
  Serial.print("  ");
  Serial.print("RB: ");
  Serial.print(ultrasonicTheoreticalDistances[ultrasonic_RB] - ultrasonicCurrentDistances[ultrasonic_RB][usmt_smooth]);
  Serial.print("  ");
  Serial.println("");
}
...
int getWallStates()
{
  ...
  if (normalWallPresence[wall_front])
    wallStates |= 0b100;
  if (normalWallPresence[wall_left])
    wallStates |= 0b010;
  if (normalWallPresence[wall_right])
    wallStates |= 0b001;
  return wallStates;
}
...
void checkSmoothWallChange(int sensor)
{
  if (currentSensorWallStates[sensor][usmt_smooth] != previousSensorWallStates[sensor][usmt_smooth])
  {
    ...
    g_smoothWallChanges[sensor] = wallchange_none;
}
...
void checkAndUseWallChange(int sensor, WallChangeType wallChangeToCheck, StoppingReason &stopReason)
{
  ...
  if (sensor == ultrasonic_LB || sensor == ultrasonic_RB)
    ultrasonicGroup = ultrasonics_back;
...
void checkWallChanges(StoppingReason &stopReason)
{
  ...
  for (int i = 0; i < 2; ++i)
  {
    ...
    if (i == 1)
      wallChangeToCheck = wallchange_leaving;
...
void printPotWallChanges()
{
  Serial.print("APPR  ");
  Serial.print("LF:");
  Serial.print(g_potWallChanges[ultrasonic_LF][wallchange_approaching].detected);
  Serial.print(" ");
  Serial.print("LB:");
  Serial.print(g_potWallChanges[ultrasonic_LB][wallchange_approaching].detected);
  Serial.print(" ");
  Serial.print("RF:");
  Serial.print(g_potWallChanges[ultrasonic_RF][wallchange_approaching].detected);
  Serial.print(" ");
  Serial.print("RB:");
  Serial.print(g_potWallChanges[ultrasonic_RB][wallchange_approaching].detected);
  Serial.print(" ");
  Serial.println("");

  Serial.print("LEAV  ");
  Serial.print("LF:");
  Serial.print(g_potWallChanges[ultrasonic_LF][wallchange_leaving].detected);
  Serial.print(" ");
  Serial.print("LB:");
  Serial.print(g_potWallChanges[ultrasonic_LB][wallchange_leaving].detected);
  Serial.print(" ");
  Serial.print("RF:");
  Serial.print(g_potWallChanges[ultrasonic_RF][wallchange_leaving].detected);
  Serial.print(" ");
  Serial.print("RB:");
  Serial.print(g_potWallChanges[ultrasonic_RB][wallchange_leaving].detected);
  Serial.print(" ");
  Serial.println("");
}
...
void printWallchangeData(UltrasonicSensorEnum sensor)
{
  ...
  Serial.print("   ");
  Serial.print("POT  ");
  Serial.print("CurrentPresent:");
  Serial.print(currentSensorWallStates[sensor][usmt_raw]);
  Serial.print(" ");
  Serial.print("PreviousPresent:");
  Serial.print(previousSensorWallStates[sensor][usmt_raw]);
  Serial.print(" ");
  Serial.print("   ");

  Serial.print("APPR: ");
  Serial.print("DETECT: ");
  Serial.print(g_potWallChanges[sensor][wallchange_approaching].detected);
  Serial.print(" ");
  Serial.print("ShadowDist: ");
  Serial.print(g_potWallChanges[sensor][wallchange_approaching].shadowDistanceDriven);
  Serial.print(" ");
  Serial.print("TIME: ");
  Serial.print(millis() - g_potWallChanges[sensor][wallchange_approaching].timestamp);
  Serial.print(" ");
  Serial.print("   ");

  Serial.print("LEAV: ");
  Serial.print("DETECT: ");
  Serial.print(g_potWallChanges[sensor][wallchange_leaving].detected);
  Serial.print(" ");
  Serial.print("ShadowDist: ");
  Serial.print(g_potWallChanges[sensor][wallchange_leaving].shadowDistanceDriven);
  Serial.print(" ");
  Serial.print("TIME: ");
  Serial.print(millis() - g_potWallChanges[sensor][wallchange_leaving].timestamp);
  Serial.print(" ");
  Serial.println("");
}
...
// Checking for interrupts
...
#endif
  {
    if (onRamp == false && g_driveBack == false)
    {
      ...
      else if (intrCommand == command_none)
        sounds::errorBeep();
      else
      {
    ...
    else // If on a ramp or driving backwards
    {
      ...
      lights::setColour(0, colourRed, true); // Light up red
    }
  }
...
bool driveStep(FloorColour &floorColourAhead, bool &rampDriven, TouchSensorSide &frontSensorDetectionType, double &xDistanceOnRamp, double &yDistanceOnRamp, bool continuing)
{
  ...
  if (stoppingReason == stop_frontTouchBoth || stoppingReason == stop_frontTouchLeft || stoppingReason == stop_frontTouchRight)
  {
    ...
    if (sensorActivation == g_lastTouchSensorState || sensorActivation == touch_both || g_lastTouchSensorState == touch_both)
    {
  ...
  {
    pose.update(wall_none); // Fast update for angle (to know if correction should be done)
  ...
  if (g_trueDistanceDriven > 15 && stoppingReason != stop_floorColour)
    return true;
  else
    return false;
}
...
{
  if (nextAction == command_none)
  { // If the given nextAction was nothing (x, -1)
    getUltrasonics();
    checkWallPresence();
    if (normalWallPresence[wall_left] && normalWallPresence[wall_front])
      action = command_turnRight;
    else if (normalWallPresence[wall_left])
      action = command_driveStep; // If the left wall is there and not the front wall
    else
    { // If the left wall disappears for any reason, including when the front wall is present
